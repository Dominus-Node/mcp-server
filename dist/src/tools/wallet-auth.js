...
export function registerWalletAuthTools(server, httpClient) {
    ...
    server.tool("dominusnode_wallet_setup", "One-shot wallet setup: request challenge, verify signature, and create an API key in one call. The agent must sign the challenge message between the challenge and verify steps. Returns proxy configuration ready to use.", {
        address: z.string().regex(/^0x[0-9a-fA-F]{40}$/).describe("Your Ethereum wallet address (0x...)"),
        ...
        try {
            ...
            const keyData = await httpClient.post("/api/keys", { label: args.key_label });
